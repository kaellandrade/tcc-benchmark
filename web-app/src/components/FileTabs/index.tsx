...
export function FileTabs({ tabs, activeTab, onTabClick, onTabClose }: FileTabsProps) {
  return (
      <div className="flex items-center gap-1 overflow-x-auto px-4 py-2 landscape:py-0.5 bg-card">
        {tabs.map((tab) => (
            <button
                key={tab.id}
                onClick={() => onTabClick(tab.id)}
                className={cn(
                    "flex items-center gap-2 px-3 py-1.5 landscape:py-1 rounded-md text-paragraph-small whitespace-nowrap transition-colors cursor-pointer",
                    activeTab === tab.id
